...
function roundingSolutionNew25!(vg::Vector{tGenerateur}, k::Int64, c1::Array{Int,1}, c2::Array{Int,1}, d::tListDisplay, nonFeasibleRoundedSolutions::Vector)
...
#    if length(vg[k].sInt.x) ≤ 20 @show vg[k].sInt.x end
    ...
    if generateurVisualise == -1 
        ...
        push!(d.XInt,vg[k].sInt.y[1])
        push!(d.YInt,vg[k].sInt.y[2])
        push!(nonFeasibleRoundedSolutions, vg[k].sInt)
        return vg[k].sInt
    elseif generateurVisualise == k
        # archivage seulement pour le generateur k
        push!(d.XInt,vg[k].sInt.y[1])
